...
# Add you own preferred extensions to ../modules/user-extensions.nix
{
  ...
  enableDjangoExtensions ? true,
  enablePythonExtensions ? true,
}:
let
  ...
  djangoExtensions = if enableDjangoExtensions then import ./django.nix { inherit pkgs; } else [];
  allExtensions = gitExtensions ++ htmlCssExtensions ++ jsTsExtensions ++ markdownExtensions ++ userExtensions ++ universalExtensions ++ pythonExtensions ++ djangoExtensions ;
